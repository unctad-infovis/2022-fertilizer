...
const f = chroma.scale(['rgba(174, 162, 154, 0.4)', 'rgba(124, 112, 103, 0.4)', 'rgba(0, 119, 184, 0.6)', '#009edb', '#009edb']).domain([2.8, 2.7, 1.9, 1.3, 1]);
const margin = {
  top: 0, right: 0, bottom: 0, left: 0
};
const inner_radius = 0;
const outer_radius = 300;
const my_domain = [0, 0.2];
const legend_ring_points = [0.1, 0.15, 0.2];
...
function App() {
    ...
    chart_elements.append('g')
      ...
      .each((domain, i, nodes) => {
        ...
        d3.select(nodes[i]).append('text')
          .attr('data-id', d1 => d1.region)
          .attr('x', d1 => ((x(d1.id) + x.bandwidth() / 2 + Math.PI) % (2 * Math.PI) < Math.PI ? -y(my_domain[1]) - 10 : y(my_domain[1]) + 10))
          .attr('y', 0)
          .text(d => d.region)
  ...
  const createRadialChart = (data) => {
    ...
    // Export SVG
    d3.select('button#export').on('click', () => {
      const config = {
  ...
  return (
    <div className="app">
      <div className="heading_container">
        <h1>
          In Africa countries are in urgent need
          <br />
          of fertilizer but lack supplies
        </h1>
      </div>
      <div className="chart_container" id="svg" />
      <div className="scales_container">
        {
          // The scale on the right.
          scales.map(scale => (<div key={scale} className="scale_container" style={{ backgroundColor: f(scale) }} />))
        }
      </div>
      <div className="meta_container">
        ...
        <div>
          Source:
          <a href="https://unctad.org">UNCTAD</a>
        </div>
      </div>
      <div className="tooltip" />
      <div><button id="export" type="button">Export SVG</button></div>
    </div>
  );
}
export default App;
